...
# Add project root to path so `from src import shorts` resolves when this file
# is run directly as a subprocess (relative imports don't work in scripts).
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from src import shorts as shorts_module

CONFIG_FILE = os.path.join(PROJECT_ROOT, "config.ini")
DEFAULT_CONFIG = {
    'PATHS': {
        'input_video': 'my_stream.mp4',
        'highlights_output': 'outputs/highlightvid.mp4',
        'montage_output': 'outputs/TrimmedVOD.mp4',
...
def extract_to_ram(track_index, name, input_video, stream_count):
...
def render(segs, out_file, desc, input_video):
    if not segs: return
    cp = os.path.join(PROJECT_ROOT, "concat_list_temp.txt")
    with open(cp, "w") as f:
...
def process_vod():
    ...
    b_word = cfg.get('BOOKMARK', 'codeword').lower().strip()
    ...
    y_mic, sr = extract_to_ram(mic_idx, "Mic", input_video, stream_count)
    y_game, _ = extract_to_ram(game_idx, "Game", input_video, stream_count)
    ...
    if num_shorts > 0:
        ...
        shorts_module.generate_shorts(ranked, input_video, num_shorts, t_spread, thumb_dir, output_dir=os.path.join("outputs", "Shorts"))
        beat.stop()

    print("\n>>> 4/4: Lossless Video Rendering...")
    render(final_vod, montage_out, "Trimmed VOD", input_video)
    render(final_h, highlights_out, "Highlight Reel", input_video)
