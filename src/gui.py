...
class AutoVodGUI(ctk.CTk):
    ...
    def save_and_run(self):
        ...
        script_path = os.path.join(PROJECT_ROOT, "src", "engine.py")
        if not os.path.exists(script_path):
            messagebox.showerror("Error", f"Engine file not found: {script_path}")
            return

        if os.name == 'nt':
            subprocess.Popen(['cmd.exe', '/k', 'python', script_path], creationflags=subprocess.CREATE_NEW_CONSOLE, cwd=PROJECT_ROOT)
        else:
